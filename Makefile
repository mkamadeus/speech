...
train: setup step1 step2 step4 step5 step6
.PHONY: train

# Tutorial taken from http://voxforge.org/home/dev/acousticmodels/linux/create/htkjulius/tutorial/download
...
step2:
	julia scripts/prompts2wlist.jl prompts.txt
...
step4:
	julia scripts/prompts2mlf.jl prompts.txt
...
# lol this be broken lmao
	for i in $(shell seq 0 2);	do \
		NEXT := `expr $i + 1`
		HERest -A -D -T 1 -C config/train_config -I temp/transcription/phones0.mlf -t 250.0 150.0 1000.0 -S temp/hmm/train.scp -H temp/hmm/hmm$$i/macros -H temp/hmm/hmm$$i/hmmdefs -M temp/hmm/hmm$$NEXT temp/pronunciation/monophones0; \
	done
